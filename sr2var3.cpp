...
Results gillespie_algorithm(ld Tmax, ld a, ld b, ld c, ld d, ll preys0,
  ...
  */
  ...
  while (t < Tmax) {
    ...
    au = a * preys_now; // скорость роста травы
    buv = b * preys_now * predators_now; // скорость поедания травы скотом
    cu = c * preys_now;     // скорость прихода скота
...
bool saveToPythonScript(Results r1, Results r2, Results r3, Resultsf rf, string s) {
  ofstream fout(s);
  if (fout.is_open()) {
    fout << "#!/bin/python3"
    ...
    fout << "t1 = [";
    for (ll i = 0; i < r1.t.size(); i++)
      fout << (i ? "," : "") + to_string(r1.t[i]);
    fout << "]" << endl << "preys1 = [";
    for (ll i = 0; i < r1.u.size(); i++)
      fout << (i ? "," : "") + to_string(r1.u[i].preys);
    fout << "]" << endl << "predators1 = [";
    for (ll i = 0; i < r1.u.size(); i++)
      fout << (i ? "," : "") + to_string(r1.u[i].predators);
    fout << "]" << endl;
        fout << "t2 = [";
    for (ll i = 0; i < r2.t.size(); i++)
      fout << (i ? "," : "") + to_string(r2.t[i]);
    fout << "]" << endl << "preys2 = [";
    for (ll i = 0; i < r2.u.size(); i++)
      fout << (i ? "," : "") + to_string(r2.u[i].preys);
    fout << "]" << endl << "predators2 = [";
    for (ll i = 0; i < r2.u.size(); i++)
      fout << (i ? "," : "") + to_string(r2.u[i].predators);
    fout << "]" << endl;
        fout << "t3 = [";
    for (ll i = 0; i < r3.t.size(); i++)
      fout << (i ? "," : "") + to_string(r3.t[i]);
    fout << "]" << endl << "preys3 = [";
    for (ll i = 0; i < r3.u.size(); i++)
      fout << (i ? "," : "") + to_string(r3.u[i].preys);
    fout << "]" << endl << "predators3 = [";
    for (ll i = 0; i < r3.u.size(); i++)
      fout << (i ? "," : "") + to_string(r3.u[i].predators);
    fout << "]" << endl;
    ...
    fout << "plt.plot(t1, preys1, label = \"Stochastic 1 - Grass\", color = \"g\")" << endl;
    fout << "plt.plot(t1, predators1, label = \"Stochastic 1 - Cows\", color = \"r\")" << endl;

    fout << "plt.plot(t2, preys2, label = \"Stochastic 2 - Grass\", color = \"b\")" << endl;
    fout << "plt.plot(t2, predators2, label = \"Stochastic 2 - Cows\", color = \"y\")" << endl;

    fout << "plt.plot(t3, preys3, label = \"Stochastic 3 - Grass\", color = \"c\")" << endl;
    fout << "plt.plot(t3, predators3, label = \"Stochastic 3 - Cows\", color = \"m\")" << endl;

    fout << "plt.plot(ode_t, ode_preys, label = \"ODE Grass\", color = \"k\", "
            "ls = \"dashed\")"
         << endl;
...
int main() {
  ...
  ld d = 0.15;
  string s = "example.py";
  auto r1 = gillespie_algorithm(100, a, b, c, d, 20, 1);
  auto r2 = gillespie_algorithm(100, a, b, c, d, 20, 1);
  auto r3 = gillespie_algorithm(100, a, b, c, d, 20, 1);
  auto rf = ode_algorithm(100, a, b, c, d, 20, 1, 1e-6, 100);
  if (saveToPythonScript(r1,r2,r3, rf, s))
    runPythonScript(s);
  return 0;
}